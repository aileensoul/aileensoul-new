...
function get_states()
{
	$sql = "select * from state where state_status != 3";
...
function get_state_bycountry($store_country)
{
	$sql = "select state_id,state_name from state where country_id='".$store_country."' and state_status = 1";
	$results = get_results($sql);
	return $results;
}
