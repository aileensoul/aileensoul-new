function checkvalue(){var a=$.trim(document.getElementById("tags").value),b=$.trim(document.getElementById("searchplace").value);return""==a&&""==b?!1:void 0}function check(){var a=$.trim(document.getElementById("tags1").value),b=$.trim(document.getElementById("searchplace1").value);return""==a&&""==b?!1:void 0}$.validator.addMethod("regx",function(a,b,c){return a?c.test(a):!0},"Only space, only number and only special characters are not allow");
$(document).ready(function(){$("#businessinfo").validate({rules:{companyname:{required:!0,regx:/^[-@.\/#&+,\w\s]*[a-zA-Z][a-zA-Z0-9]*/},country:{required:!0},state:{required:!0},business_address:{required:!0,regx:/^[-@.\/#&+,\w\s]*[a-zA-Z][a-zA-Z0-9]*/}},messages:{companyname:{required:company_name_validation},country:{required:country_validation},state:{required:state_validation},business_address:{required:address_validation}}})});$(".alert").delay(3200).fadeOut(300);
...
"business_profile/ajax_data",data:"state_id="+a,success:function(a){$("#city").html(a)}}):$("#city").html('<option value="">Select state first</option>')})});$(document).ready(function(){var a=$("#business_address"),b=a.val().length;a[0].focus();a[0].setSelectionRange(b,b);a=$("#companyname");b=a.val().length;a[0].focus();a[0].setSelectionRange(b,b)});
