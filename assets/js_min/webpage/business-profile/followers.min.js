$(document).ready(function(){business_followers(slug_id);$(window).scroll(function(){if($(window).scrollTop()>=.7*($(document).height()-$(window).height())){var a=$(".page_number:last").val(),b=$(".total_record").val(),c=$(".perpage_record").val();if(parseInt(c)<=parseInt(b)){var d=parseInt(b/c,10);0<b%c&&(d+=1);parseInt(a)<=parseInt(d)&&(a=parseInt($(".page_number:last").val())+1,business_followers(slug_id,a))}}})});var isProcessing=!1;
function business_followers(a,b){isProcessing||(isProcessing=!0,$.ajax({type:"POST",url:base_url+"business_profile/ajax_followers/"+a+"?page="+b,data:{total_record:$("#total_record").val()},dataType:"html",beforeSend:function(){"undefined"==b?$(".contact-frnd-post").prepend('<p style="text-align:center;"><img class="loader" src="'+base_url+'images/loading.gif"/></p>'):$("#loader").show()},complete:function(){$("#loader").hide()},success:function(a){$(".loader").remove();$(".contact-frnd-post").append(a);
0==$(".post-design-box").length?$("#dropdownclass").addClass("no-post-h2"):$("#dropdownclass").removeClass("no-post-h2");isProcessing=!1}}))}function checkvalue(){var a=$.trim(document.getElementById("tags").value),b=$.trim(document.getElementById("searchplace").value);if(""==a&&""==b)return!1}function check(){var a=$.trim(document.getElementById("tags1").value),b=$.trim(document.getElementById("searchplace1").value);if(""==a&&""==b)return!1}
function followuser_two(a){$.ajax({type:"POST",url:base_url+"business_profile/follow_two",data:"follow_to="+a,success:function(b){$("#frfollow"+a).html(b)}})}function unfollowuser_two(a){$.ajax({type:"POST",url:base_url+"business_profile/unfollow_two",data:"follow_to="+a,success:function(b){$("#frfollow"+a).html(b)}})}$(document).ready(function(){$("html,body").animate({scrollTop:330},500)});
...
a+" onClick='contact_person_menu("+a+")' href='javascript:void(0);' data-dismiss='modal'>Yes</a><a class='cnclbtn' href='javascript:void(0);' data-dismiss='modal'>No</a></div></div>"),$("#bidmodal").modal("show"))}function contact_person_query(a,b){$.ajax({type:"POST",url:base_url+"business_profile/contact_person_query",data:"toid="+a+"&status="+b,success:function(c){contact_person_model(a,b,c)}})}
...
function contact_person(a){$.ajax({type:"POST",url:base_url+"business_profile/contact_person",data:"toid="+a,success:function(a){$("#contact_per").html(a)}})}$(document).on("keydown",function(a){27===a.keyCode&&$("#query").modal("hide")});
