function checkvalue(){var a=$.trim(document.getElementById("tags").value),c=$.trim(document.getElementById("searchplace").value);if(""==a&&""==c)return!1}$.validator.addMethod("regx1",function(a,c,b){return b.test(a)},"Only numbers are allowed.");$.validator.addMethod("regx",function(a,c,b){return b.test(a)},"Only space and only number are not allow.");
$(document).ready(function(){$("#contactinfo").validate({rules:{contactname:{required:!0,regx:/^[a-zA-Z\s]*[a-zA-Z]/},contactmobile:{required:!0,number:!0,minlength:8,maxlength:15},email:{required:!0,email:!0,remote:{url:base_url+"business_profile/check_email",type:"post",data:{email:function(){return $("#email").val()},get_csrf_token_name:get_csrf_hash}}}},messages:{contactname:{required:"Person name is required."},contactmobile:{required:"Mobile number is required."},email:{required:"Email id is required.",
email:"Please enter valid email id.",remote:"Email already exists."}}})});$(".alert").delay(3200).fadeOut(300);function check(){var a=$.trim(document.getElementById("tags1").value),c=$.trim(document.getElementById("searchplace1").value);if(""==a&&""==c)return!1}$(document).ready(function(){var a=$("#contactname"),c=a.val().length;a[0].focus();a[0].setSelectionRange(c,c)});
$(function(){$(".tags").bind("keydown",function(a){a.keyCode===$.ui.keyCode.TAB&&$(this).autocomplete("instance").menu.active&&a.preventDefault()}).autocomplete({minLength:1,source:function(a,c){$.getJSON(base_url+"business_profile/ajax_business_skill",{term:a.term.split(/,\s*/).pop()},c)},focus:function(){return!1},select:function(a,c){var b=this.value.split(/,\s*/);1>=b.length?(b.pop(),b.push(c.item.value),b.push(""),this.value=b.join("")):(b=b.pop(),$(this).val(this.value.substr(0,this.value.length-
b.length-2)),$(this).effect("highlight",{},1E3),$(this).attr("style","border: solid 1px red;"));return!1}});$(".searchplace").bind("keydown",function(a){a.keyCode===$.ui.keyCode.TAB&&$(this).autocomplete("instance").menu.active&&a.preventDefault()}).autocomplete({minLength:1,source:function(a,c){$.getJSON(base_url+"business_profile/ajax_location_data",{term:a.term.split(/,\s*/).pop()},c)},focus:function(){return!1},select:function(a,c){var b=this.value.split(/,\s*/);1>=b.length?(b.pop(),b.push(c.item.value),
b.push(""),this.value=b.join("")):(b=b.pop(),$(this).val(this.value.substr(0,this.value.length-b.length-2)),$(this).effect("highlight",{},1E3),$(this).attr("style","border: solid 1px red;"));return!1}})});$uploadCrop1=$("#upload-demo-one").croppie({enableExif:!0,viewport:{width:157,height:157,type:"square"},boundary:{width:257,height:257}});
...
function readURL(a){if(a.files&&a.files[0]){var c=new FileReader;c.onload=function(a){document.getElementById("preview").style.display="block";$("#preview").attr("src",a.target.result)};c.readAsDataURL(a.files[0])}}$("#profilepic").change(function(){profile=this.files;if(profile[0].name.match(/.(jpg|jpeg|png|gif)$/i))readURL(this);else return $("#profilepic").val(""),picpopup(),!1});
