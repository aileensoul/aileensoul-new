...
$(document).ready(function(){business_userlist();$(window).scroll(function(){if($(window).scrollTop()>=.7*($(document).height()-$(window).height())){var a=$(".page_number:last").val(),b=$(".total_record").val(),c=$(".perpage_record").val();if(parseInt(c)<=parseInt(b)){var d=parseInt(b/c,10);0<b%c&&(d+=1);parseInt(a)<=parseInt(d)&&(a=parseInt($(".page_number:last").val())+1,business_userlist(a))}}})});var isProcessing=!1;
function business_userlist(a){isProcessing||(isProcessing=!0,$.ajax({type:"POST",url:base_url+"business_profile/ajax_userlist/?page="+a,data:{total_record:$("#total_record").val()},dataType:"html",beforeSend:function(){"undefined"!=a&&$("#loader").show()},complete:function(){$("#loader").hide()},success:function(a){$(".loader").remove();$(".contact-frnd-post").append(a);0==$(".post-design-box").length?$("#dropdownclass").addClass("no-post-h2"):$("#dropdownclass").removeClass("no-post-h2");isProcessing=
!1}}))}$(document).ready(function(){$("html,body").animate({scrollTop:330},500)});function followuser(a){$.ajax({type:"POST",url:base_url+"business_profile/follow",dataType:"json",data:"follow_to="+a,success:function(b){$(".fruser"+a).html(b.follow);$("#countfollow").html(b.count)}})}function unfollowuser(a){$.ajax({type:"POST",url:base_url+"business_profile/unfollow",dataType:"json",data:"follow_to="+a,success:function(b){$(".fruser"+a).html(b.follow);$("#countfollow").html(b.count)}})}
