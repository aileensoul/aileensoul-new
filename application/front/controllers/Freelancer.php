        ...
        if (count($postdata1) > 0) {
            foreach ($postdata as $post) {
                ...
                if ($retur == '' && $id == 'null') {
                    ...
                    $return_html .= '<li><a href="javascript:void(0);" class="button" onclick="removepopup(' . $post['post_id'] . ')">';
                    ...
                    $return_html .= '<a class="button" href="' . base_url('freelancer-hire/freelancer-shortlisted/' . $post['post_id']) . '" >';
                    $return_html .= 'Short listed';
                    $return_html .= ':';
...
// post detail
        ...
        $postdetail = $this->data['postdetail'] = $this->common->select_data_by_condition('freelancer_apply1', $contition_array, $data = 'freelancer_apply.app_id, freelancer_post.post_id, freelancer_post.user_id, freelancer_post.created_date, freelancer_post.post_name, freelancer_post.post_field_req, freelancer_post.post_est_time, freelancer_post.post_skill, freelancer_post.post_exp_month, freelancer_post.post_exp_year, freelancer_post.post_other_skill, freelancer_post.post_description, freelancer_post.post_rate, freelancer_post.post_last_date, freelancer_post.post_currency, freelancer_post.post_rating_type, freelancer_post.country, freelancer_post.city', $sortby = 'freelancer_apply.modify_date', $orderby = 'desc', $limit, $offset, $join_str, $groupby = '');
