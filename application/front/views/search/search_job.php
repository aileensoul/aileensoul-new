...
<br/><br/> -->
                        <div>
                            ...
                            if($falguni == 1)
                            {
                                //echo"tank"; die();
                                    // List up all results.
                                    foreach ($jobdata as $job_key => $job_val)
                                    {
                                        foreach ($job_val as $jobval)
                                        {
                                            ?>

                                        <div>
                                        <?php
                                        $cache_time  =  $this->db->get_where('job_reg',array('user_id' => $jobval['user_id']))->row()->job_user_image;
                                        ?>
                                       <a href="<?php echo base_url('recruiter/recruiter_postdata/'.$jobval['user_id']); ?>"><img src="<?php echo base_url(USERIMAGE.$cache_time)?>" style="width:50px;height:50px;"></a>
                                        </div>

                                        <div>
                                        <a href="<?php echo base_url('recruiter/recruiter_postdata/'.$jobval['user_id']); ?>">
                                        <?php 
                                        
                                        echo ucwords($jobval['rec_firstname']) . ' ' . ucwords($jobval['rec_lastname']); 
                                        ?></a>
                                        </div>
                                        ...
                                        $searchskill = explode(',',$jobval['post_skill']);
                                       ...
                                       // print_r($cache_time);die();
                                        //foreach ($cache_time as $value) {
                                         ...
                                         <div>
                                         <?php
                                        $cache_time  =  $this->db->get_where('cities',array('city_id' => $jobval['re_comp_city']))->row()->city_name;  
                                        echo $cache_time; 
                                        ?>
                                         </div> 
                                         <?php     
