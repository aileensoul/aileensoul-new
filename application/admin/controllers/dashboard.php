...
class Dashboard extends MY_Controller {
    ...
    public function index() {
        ...
        $page_condition=array('status !='=>3);
        $this->data['page_count']=  count($this->common->select_data_by_condition('pages', $page_condition,'id','','','','',array()));
        
        $category_condition=array('status !='=>3);
        $this->data['category_count']=  count($this->common->select_data_by_condition('category', $category_condition,'id','','','','',array()));
        
        $manufacturers_condition=array('status !='=>3);
        $this->data['manufacturers_count']=  count($this->common->select_data_by_condition('manufacturers', $manufacturers_condition,'id','','','','',array()));
        
        $product_condition=array('status !='=>3);
        $this->data['product_count']=  count($this->common->select_data_by_condition('products', $product_condition,'id','','','','',array()));
        
        $package_condition=array('status !='=>3);
        $this->data['package_count']=  count($this->common->select_data_by_condition('bid_package', $package_condition,'package_id','','','','',array()));
        
        $deal_condition=array('status !='=>3);
        $this->data['deal_count']=  count($this->common->select_data_by_condition('deals_promotions', $deal_condition,'id','','','','',array()));
        
        $local_condition=array('status !='=>3);
        $this->data['local_count']=  count($this->common->select_data_by_condition('local_community', $local_condition,'id','','','','',array()));
        
        $testimonials_condition=array('status !='=>3);
        $this->data['testimonials_count']=  count($this->common->select_data_by_condition('testimonials', $testimonials_condition,'id','','','','',array()));
        
        $videos_condition=array('status !='=>3);
        $this->data['videos_count']=  count($this->common->select_data_by_condition('videos', $videos_condition,'id','','','','',array()));
        
        $users_condition=array('status !='=>3);
        $this->data['users_count']=  count($this->common->select_data_by_condition('users', $users_condition,'id','','','','',array()));
